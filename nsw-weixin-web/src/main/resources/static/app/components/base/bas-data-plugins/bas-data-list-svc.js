				...
				service.loadSearchData = function getSearchList(userOptions) {
					...
					var promise = $http(queryOptions).then(function (res) {
						...
						if (data.isSuccess) {
							...
							var dataList = data.dataList || data.listData;
							if (enablePaging && dataList) {
								local.dataList = dataList; //后端有时返回dataList, 有时候返回listData
								local.searchOptions.totalPages = data.totalPages || data.pageCount;
								local.searchOptions.totalRows = data.totalRows || data.totalCount;
								local.searchOptions.pageNum =  data.pageNumber || local.searchOptions.pageNum || 1;
