	...
	angular.module('platform').factory('basDataItemSvc', ['$http', '$q', 'platformMessenger', 'platformModalSvc', 'nswGlobals',
		function ($http, $q, PlatformMessenger, platformModalSvc, nswGlobals) {
			return function (service, local) {
				 ...
				 * @param item 被保存的项
				 */
				service.saveCreate = function saveCreate(item, userOptions,logInfo) {
					logInfo = logInfo||'';
					...
					var options = {
						...
						data: itemToSave,
						headers: {'logContent':encodeURI(logInfo)}
					};
				 ...
				 * @param item 被保存对象
				 */
				service.updateItem = function updateItem(item, userOptions,logInfo) {
					logInfo = logInfo||'';
					item = item || local.currentItem;
					userOptions = userOptions || {};
					userOptions.displayTip = _.isUndefined(userOptions.displayTip) ? true : userOptions.displayTip;
				 ...
				 * @param item 被删除的项
				 */
				service.removeItem = function removeItem(item,logInfo) {
					logInfo = logInfo|| '';
					item = item || local.currentItem;
					var options = {
						method: 'delete',
